...
}, xhttp.open("GET", "assets/json/api-key-list.json"), xhttp.send(), document.getElementById("api-key-modal").addEventListener("show.bs.modal", function(e) {
...
function generateApiID() {
    ...
    return window.performance && "function" == typeof window.performance.now && (n += performance.now()), "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(e) {
        var t = (n + 16 * Math.random()) % 16 | 0;
        return n = Math.floor(n / 16), ("x" == e ? t : 3 & t | 8).toString(16)
    })
}
refreshCallbacks(), ischeckboxcheck(), document.getElementById("api-key-modal").addEventListener("hidden.bs.modal", function() {
    clearFields()
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    apiKeyList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click()
                })
            })
        })
    })
...
function clearFields() {
    apiKeyNameField.value = "", apiKeyField.value = "", document.getElementById("apikey-element").style.display = "none", document.getElementById("add-btn").style.display = "none"
}
...
document.getElementById("createApi-btn").addEventListener("click", function(e) {
    ...
    if (t.classList.remove("d-none"), setTimeout(() => document.getElementById("api-key-error-msg").classList.add("d-none"), 2e3), 0 == apiKeyNameField.value.length) return !(t.innerHTML = "Please enter api key name");
