...
}, xhttp.open("GET", "assets/json/contact-list.json"), xhttp.send(), isCount = (new DOMParser).parseFromString(contactList.items.slice(-1)[0]._values.id, "text/html"), document.querySelector("#customer-image-input").addEventListener("change", function() {
...
var idField = document.getElementById("id-field"),
    ...
    table = (viewBtns = document.getElementsByClassName("view-item-btn"), refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
        e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Contact", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Contact", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Contact") : (document.getElementById("exampleModalLabel").innerHTML = "List Contact", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
    }), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function(e) {
        clearFields()
...
var count = 11,
    ...
    forms = (Array.from(tagInputFieldValue).forEach((e, t) => {
        tagHtmlValue += '<span class="badge badge-soft-primary me-1">' + e + "</span>"
    ...
    removeBtns && Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    contactList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click()
                })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                t == itemId && (editlist = !0, idField.value = t, customerImg.src = (new DOMParser).parseFromString(e._values.name, "text/html").body.querySelector("img").src, customerNameField.value = (new DOMParser).parseFromString(e._values.name, "text/html").body.querySelector(".name").innerHTML, company_nameField.value = e._values.company_name, designationField.value = e._values.designation, email_idField.value = e._values.email_id, phoneField.value = e._values.phone, lead_scoreField.value = e._values.lead_score, a) && Array.from(a).forEach(e => {
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && (e = `
                        ...
                        </div>`, document.getElementById("contact-view-detail").innerHTML = e)
            })
        })
    })
}
...
function clearFields() {
    customerImg.src = "assets/images/users/user-dummy-img.jpg", customerNameField.value = "", company_nameField.value = "", designationField.value = "", email_idField.value = "", phoneField.value = "", lead_scoreField.value = "", tagInputField.removeActiveItems(), tagInputField.setChoiceByValue("0")
}
...
function deleteMultiple() {
    ...
    "undefined" != typeof ids_array && 0 < ids_array.length ? Swal.fire({
        ...
        if (e.value) {
            for (i = 0; i < ids_array.length; i++) contactList.remove("id", `<a href="javascript:void(0);" class="fw-medium link-primary">${ids_array[i]}</a>`);
...
Array.prototype.slice.call(forms).forEach(function(n) {
    n.addEventListener("submit", function(e) {
        var t, a;
        n.checkValidity() ? (e.preventDefault(), "" === customerNameField.value || "" === company_nameField.value || "" === email_idField.value || "" === phoneField.value || "" === lead_scoreField.value || "" === designationField.value || editlist ? "" !== customerNameField.value && "" !== company_nameField.value && "" !== email_idField.value && "" !== phoneField.value && "" !== lead_scoreField.value && "" !== designationField.value && editlist && (t = contactList.get({
            id: idField.value
            ...
            Array.from(a).forEach((e, t) => {
                n += '<span class="badge badge-soft-primary me-1">' + e + "</span>"
            }), t == itemId && e.values({
            ...
            timer: 2e3,
            showCloseButton: !0
        })) : (t = tagInputField.getValue(!0), a = "", Array.from(t).forEach((e, t) => {
            a += '<span class="badge badge-soft-primary me-1">' + e + "</span>"
        }), contactList.add({
