var checkAll = document.getElementById("checkAll"),
    isChoiceEl = (checkAll && (checkAll.onclick = function() {
    ...
    choices = new Choices(isTypeEl, {
        searchEnabled: !1
    }),
    perPage = 8,
    editlist = !1,
    options = {
        valueNames: ["id", "company", "designation", "date", "contacts", "type", "status"],
        page: perPage,
        pagination: !0,
    ...
    applicationList = new List("applicationList", options).on("updated", function(e) {
...
xhttp.onload = function() {
    ...
    Array.from(e).forEach(function(e) {
        applicationList.add({
            ...
            company: '<div class="d-flex align-items-center">            <div class="flex-shrink-0">                <img src="' + e.company[0] + '" alt="" class="avatar-xxs rounded-circle image_src object-cover">            </div>            <div class="flex-grow-1 ms-2 name">' + e.company[1] + "</div>        </div>",
            designation: e.designation,
            date: e.date,
            contacts: e.contacts,
            type: e.type,
            status: isStatus(e.status)
        }), applicationList.sort("id", {
            order: "desc"
        }), refreshCallbacks()
    }), applicationList.remove("id", '<a href="#" class="fw-medium link-primary">#VZ001</a>')
}, xhttp.open("GET", "assets/json/application-list.json"), xhttp.send(), document.querySelector("#companylogo-image-input").addEventListener("change", function() {
...
var example = new Choices(typeField, {
    ...
    table = (document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
        e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Application", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Application", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Application") : (document.getElementById("exampleModalLabel").innerHTML = "List Application", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
...
function filterData() {
    ...
    applicationList.filter(function(e) {
        var t = (matchData = (new DOMParser).parseFromString(e.values().status, "text/html")).body.firstElementChild.innerHTML,
            ...
            i = new Date(e.values().date.slice(0, 12)) >= new Date(c) && new Date(e.values().date.slice(0, 12)) <= new Date(d);
        return a && n && i || (a && n && "" == o ? a && n : n && i && "" == o ? n && i : void 0)
    }), applicationList.update()
}
var count = 13,
    ...
    Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    applicationList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click()
                })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                t == itemId && (editlist = !0, idField.value = t, t = (new DOMParser).parseFromString(e._values.company, "text/html").body.querySelector(".name").innerHTML, companyField.value = t, companyLogoImg.src = (new DOMParser).parseFromString(e._values.company, "text/html").body.querySelector(".image_src").src, designationField.value = e._values.designation, dateField.value = e._values.date, contactField.value = e._values.contacts, statusVal && statusVal.destroy(), statusVal = new Choices(statusField, {
                    searchEnabled: !1
                }), t = (val = (new DOMParser).parseFromString(e._values.status, "text/html")).body.firstElementChild.innerHTML, statusVal.setChoiceByValue(t), example && example.destroy(), example = new Choices(typeField, {
                    searchEnabled: !1
                }), t = e._values.type, example.setChoiceByValue(t), flatpickr("#date-field", {
                    enableTime: !1,
                    dateFormat: "d M, Y",
...
function clearFields() {
    companyField.value = "", companyLogoImg.src = "", designationField.value = "", dateField.value = "", contactField.value = "", example && example.destroy(), example = new Choices(typeField), statusVal && statusVal.destroy(), statusVal = new Choices(statusField)
}
...
function deleteMultiple() {
    ...
    "undefined" != typeof ids_array && 0 < ids_array.length ? Swal.fire({
        ...
        if (e.value) {
            for (i = 0; i < ids_array.length; i++) applicationList.remove("id", '<a href="#" class="fw-medium link-primary">' + ids_array[i] + "</a>");
...
Array.prototype.slice.call(forms).forEach(function(a) {
    a.addEventListener("submit", function(e) {
        var t;
        a.checkValidity() ? (e.preventDefault(), "" === companyField.value || "" === designationField.value || "" === dateField.value || "" === contactField.value || "" === statusField.value || "" === typeField.value || editlist ? "" !== companyField.value && "" !== designationField.value && "" !== dateField.value && "" !== contactField.value && "" !== statusField.value && "" !== typeField.value && editlist && (t = applicationList.get({
            id: idField.value
            ...
            (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && e.values({
                ...
                company: '<div class="d-flex align-items-center">                            <div class="flex-shrink-0">                                <img src="' + companyLogoImg.src + '" alt="" class="avatar-xxs rounded-circle image_src object-cover">                            </div>                            <div class="flex-grow-1 ms-2 name">' + companyField.value + "</div>                        </div>",
