...
var chart, MarketchartColors = getChartColorsArray("Market_chart"),
    perPage = (MarketchartColors && (options = {
        series: [{
            data: [{
    ...
    }, (chart = new ApexCharts(document.querySelector("#Market_chart"), options)).render()), 8),
    options = {
    ...
    marketList = new List("marketList", options).on("updated", function(e) {
...
isCount = (new DOMParser).parseFromString(marketList.items.slice(-1)[0]._values.id, "text/html"), document.querySelector(".pagination-next") && document.querySelector(".pagination-next").addEventListener("click", function() {
